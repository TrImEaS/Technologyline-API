...
async function refreshDB() {
  ...
  try {
    ...
    const mapColumnNames = (rowData) => ({
      id: parseInt(rowData[29]), // id del Excel
      ...
      sub_category: cleanCategory((rowData[26] && rowData[26].toString()) || ''),
      brand: cleanCategory((rowData[27] && rowData[27].toString()) || ''),
    ...
    const existingProductMap = new Map(existingProducts.map(product => [product.sku, product.id])); // Mapear SKU a ID de productos
    ...
    for (const excelProduct of productsExcel) {
      const { id, sku, name, stock, category, sub_category, brand, img_base, discount } = excelProduct;
      ...
      if (existingProductMap.has(sku)) {
        ...
        updateProductQueries.push(connection.query(
          'UPDATE products SET id = ?, sku = ?, name = ?, stock = ?, category = ?, sub_category = ?, brand = ?, img_base = ?, discount = ?, status = ? WHERE id = ?',
          [id, sku, name, stock, category, sub_category, brand, img_base, discount, stock < 0 ? 0 : 1, dbProductId]
        ));
      ...
      // Si el producto no existe, lo insertamos
        ...
        insertProductQueries.push(connection.query(
          'INSERT INTO products (id, sku, name, stock, category, sub_category, brand, img_base, total_views, specifications, descriptions, discount, status, adminStatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [id, sku, name, stock, category, sub_category, brand, img_base, 0, 'Este producto no contiene especificaciones', 'Este producto no contiene descripcion', discount, 1, 1]
        ));
